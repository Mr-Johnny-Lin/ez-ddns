...
func (p *AliDNSProvider) GetDomainRecords(ctx context.Context, domainConfig model.DomainConfig) ([]DNSRecord, error) {
	...
	logger.Debug("查询域名记录: %s.%s", domainConfig.RR, domainConfig.DomainName)
	...
	logger.Debug("查询到 %d 条域名记录", len(result))
	return result, nil
}
...
func (p *AliDNSProvider) AddDomainRecord(ctx context.Context, domainConfig model.DomainConfig, ip string) error {
	logger := utils.LoggerFromContext(ctx)
	logger.Debug("添加DNS记录: %s.%s -> %s", domainConfig.RR, domainConfig.DomainName, ip)
	...
	if err != nil {
		logger.Error("添加DNS记录失败: %v", err)
		return fmt.Errorf("新增DNS记录失败: %v", err)
	}

	logger.Debug("DNS记录添加成功: %s.%s -> %s", domainConfig.RR, domainConfig.DomainName, ip)
	return nil
}

func (p *AliDNSProvider) DeleteDomainRecord(ctx context.Context, recordID string) error {
	logger := utils.LoggerFromContext(ctx)
	logger.Debug("删除DNS记录: %s", recordID)
	...
	if err != nil {
		logger.Error("删除DNS记录失败: %v", err)
		return fmt.Errorf("删除DNS记录失败: %v", err)
	}

	logger.Debug("DNS记录删除成功: %s", recordID)
	return nil
}
